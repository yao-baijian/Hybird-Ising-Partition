...
def _run_mode(mode_name, fn, hyperedges, num_nodes, verbose, **kwargs):
    res = fn(hyperedges, num_nodes, q=2, coarsen_to=10, verbose=verbose, **kwargs)
...
def test_compare_coarsen_modes():
    ...
    _, kahypar_cut, kahypar_imb, kahypar_projected_cut, kahypar_projected_imb = _run_mode('kahypar_like', coarsen_kahypar_like, hyperedges, num_nodes, verbose = True)
    ...
    assert kahypar_imb >= 0 and fem_greedy_imb >= 0
    assert kahypar_projected_imb >= 0 and fem_greedy_projected_imb >= 0
